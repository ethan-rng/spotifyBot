...
const BottomRow = () => {
    ...
    const BottomButton = (icon, text) => {
        ...
        <div>
            ...
            : 
                <button className="hover:bg-primary text-gray-100 font-bold py-2 px-4 rounded transition duration-600">
                    <img className="w-10 l-10" src={ icon }></img> 
                    { text }
                </button> 
            }
    ...
    return (
        <div className="bg-gray-200 fixed bottom-0 left-0 w-screen flex justify-center">
           { links.map((link, index) => (BottomButton(link["image"], link["tag"]))) }
        </div>
  );
};
